...
/**
 ...
 - Save Codable structs, UIImage, [UIImage], Data, [Data] to Apple recommended locations on the user's storage, without having to worry about serialization.
 - Retrieve an object from storage as the type you specify, without having to worry about deserialization.
 - Remove specific objects from storage, clear entire directories if you need to, check if an object exists on storage, and much more!
...
public class Storage {
    ...
    public enum Directory: Equatable {
        ...
        /// Note that the system may delete the Caches/ directory to free up storage space, so your app must be able to re-create or download these files as needed.
        case caches
        ...
        static public func == (lhs: Directory, rhs: Directory) -> Bool {
            switch (lhs, rhs) {
            case (.documents, .documents), (.caches, .caches),
                (.applicationSupport, .applicationSupport), (.temporary, .temporary):
                return true
            case (
                let .sharedContainer(appGroupName: name1), let .sharedContainer(appGroupName: name2)
            ):
                return name1 == name2
            default:
                return false
