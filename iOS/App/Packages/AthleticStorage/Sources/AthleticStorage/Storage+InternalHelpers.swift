...
extension Storage {
    ...
    internal static func createURL(for path: String?, in directory: Directory) throws -> URL {
        let filePrefix = "file://"
        ...
        case .temporary:
            if var url = URL(string: NSTemporaryDirectory()) {
                ...
                throw createError(
                    ...
                    description:
                        "Could not create URL for \(directory.pathDescription)/\(validPath ?? "")",
        ...
        case .sharedContainer(let appGroupName):
            if var url = FileManager.default.containerURL(
                ...
                throw createError(
                    ...
                    description:
                        "Could not create URL for \(directory.pathDescription)/\(validPath ?? "")",
                    failureReason:
                        "Could not get access to shared container with app group named \(appGroupName).",
                    recoverySuggestion:
                        "Check that the app-group name in the entitlement matches the string provided."
                )
            }
        }
    ...
    static func getExistingFileURL(for path: String?, in directory: Directory) throws -> URL {
        do {
            ...
            throw createError(
                ...
                recoverySuggestion:
                    "Check if a file or folder exists before trying to commit an operation on it."
            )
        } catch {
            throw error
        }
    ...
    internal static func getValidFilePath(from originalString: String) throws -> String {
        var invalidCharacters = CharacterSet(charactersIn: ":")
        invalidCharacters.formUnion(.newlines)
        invalidCharacters.formUnion(.illegalCharacters)
        invalidCharacters.formUnion(.controlCharacters)
        let pathWithoutIllegalCharacters =
            originalString
        ...
        guard validFileName.count > 0 && validFileName != "." else {
            throw createError(
                ...
                failureReason:
                    "Cannot write/read a file with the name \(originalString) on storage.",
    ...
    internal static func removeSlashesAtBeginning(of string: String) -> String {
        var string = string
        if string.prefix(1) == "/" {
    ...
    internal static func setIsExcludedFromBackup(
        to isExcludedFromBackup: Bool,
        for path: String?,
    ...
    /// Set 'isExcludedFromBackup' BOOL property of a file or directory in the file system
    internal static func setIsExcludedFromBackup(to isExcludedFromBackup: Bool, for url: URL) throws
    {
        do {
            var resourceUrl = url
            var resourceValues = URLResourceValues()
    ...
    internal static func createSubfoldersBeforeCreatingFile(at url: URL) throws {
        do {
    ...
    internal static func fileNameInt(_ url: URL) -> Int? {
