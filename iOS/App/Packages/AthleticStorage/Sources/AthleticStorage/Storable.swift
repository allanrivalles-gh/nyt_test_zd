...
public struct StorageMetadata: StorageMetadataProtocol, Codable {
    ...
    public static func getMetadataFilepath(for keyName: String) -> String {
        StorageMetadata.basePath + keyName + StorageMetadata.metadataExtension
            + StorageMetadata.fileExtension
    }
...
public struct Storable<Value: Codable> {
    ...
    public var wrappedValue: Value {
        get {
            ...
            if let value = try? Storage.retrieve(
                wrappedValueFileName,
    ...
    public var metaData: StorageMetadata {
        get {
            guard
                let metaData = try? Storage.retrieve(
                    metaDataFileName,
                    from: .applicationSupport,
                    as: StorageMetadata.self
                )
            else {
