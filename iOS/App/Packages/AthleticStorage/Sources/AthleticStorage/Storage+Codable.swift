...
extension Storage {
    /// Save encodable struct to storage as JSON data
    ///
    /// - Parameters:
    ...
    /// - Throws: Error if there were any issues encoding the struct or writing it to storage
    ...
    ///
    /// - Parameters:
    ///   - value: the struct to store to storage
    ///   - path: file location to store the data (i.e. "Folder/file.json")
    ///   - directory: user directory to store the file in
    ///   - decoder: custom JSONDecoder to decode existing values
    ///   - encoder: custom JSONEncoder to encode new value
    /// - Throws: Error if there were any issues with encoding/decoding or writing the encoded struct to storage
    public static func append<T: Codable>(
        _ value: T,
        to path: String,
        ...
        do {
            guard let url = try? getExistingFileURL(for: path, in: directory) else {
                return try save([value], to: directory, as: path, encoder: encoder)
            }
            ...
            if !(oldData.count > 0) {
                ...
                if let old = try? decoder.decode(T.self, from: oldData) {
                    ...
                    throw createDeserializationErrorForAppendingStructToInvalidType(
                        url: url,
                        type: value
                    )
                }
    ...
    ///   - decoder: custom JSONDecoder to decode existing values
    ///   - encoder: custom JSONEncoder to encode new value
    /// - Throws: Error if there were any issues writing the encoded struct array to storage
    public static func append<T: Codable>(
        _ value: [T],
        to path: String,
        in directory: Directory,
    ...
    /// Retrieve and decode a struct from a file on storage
    ///
    /// - Parameters:
    ...
    public static func retrieve<T: Decodable>(
        _ path: String,
...
extension Storage {
    ...
    fileprivate static func createDeserializationErrorForAppendingStructToInvalidType<T>(
        url: URL,
        type: T
    ) -> Error {
        return Storage.createError(
            .deserialization,
            description:
                "Could not deserialize the existing data at \(url.path) to a valid type to append to.",
            failureReason:
                "JSONDecoder could not decode type \(T.self) from the data existing at the file location.",
            recoverySuggestion:
                "Ensure that you only append data structure(s) with the same type as the data existing at the file location."
        )
    }
