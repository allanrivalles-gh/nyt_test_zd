 ...
 *
 * Modified version of https://github.com/leprosus/kotlin-hashids
 */
class IDHasher(
    entity: Entity,
    ...
    companion object {
        const val defaultSalt = "1CnGYuU4wwaseYPbWuEHM9fCTscaf7wCQ1"
        const val defaultMinimalHashLength = 12
        const val defaultAlphabet = "0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz"
        ...
        fun forUser() = IDHasher(Entity.USER)
    }

    enum class Entity(val entityName: String) {
        USER("user"),
    }

    private val finalSalt = whatSalt("${entity.entityName}$salt${entity.entityName}")
