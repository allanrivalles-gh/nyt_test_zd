 ...
 */
class TestNoTeamLeagueSelectionInOB(
    override var name: String = "No team/league selection in OB",
    override var exists: Boolean = false,
    override var activeVariant: Variant? = NoTeamLeagueSelectionInOBVariant.CTRL(),
   ...
   * config
   */
  val variant: NoTeamLeagueSelectionInOBVariant
    get() {
      ...
      if (debugPreferences.compassSelectedVariantMap[name] != null) {
        return when(debugPreferences.compassSelectedVariantMap[name]) {
          "A" -> NoTeamLeagueSelectionInOBVariant.A()
          "B" -> NoTeamLeagueSelectionInOBVariant.B()
          else -> NoTeamLeagueSelectionInOBVariant.CTRL()
        }
      }

      if (exists) {
        return activeVariant as NoTeamLeagueSelectionInOBVariant
      }

      return NoTeamLeagueSelectionInOBVariant.CTRL()
    }

  override fun copy(activeVariant: Variant, exists: Boolean) =
      TestNoTeamLeagueSelectionInOB(activeVariant = activeVariant, exists = exists, crashLogHandler
  ...
  sealed class NoTeamLeagueSelectionInOBVariant : Variant {
    data class CTRL(
      override val _name: String = "CTRL"
    ) : NoTeamLeagueSelectionInOBVariant() {
