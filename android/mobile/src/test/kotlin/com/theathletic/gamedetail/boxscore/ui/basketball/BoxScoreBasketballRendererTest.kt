...
import com.theathletic.gamedetail.boxscore.ui.BASKETBALL_PLAY_BY_PLAY_MODULE_ID
...
class BoxScoreBasketballRendererTest {

    private lateinit var renderer: BoxScoreBasketballRenderer
    ...
    private val mockBasketballPlayByPlayRenderers: BasketballPlayByPlayRenderers = mock()
    private val mockBoxScorePlayerGradeRenderers: BoxScorePlayerGradeRenderers = mock()
    ...
    fun setup() {
        ...
        renderer = BoxScoreBasketballRenderer(
            ...
            mockBasketballPlayByPlayRenderers,
            mockSupportedLeagues,
            mockBoxScorePlayerGradeRenderers,
        ...
        whenever(mockBasketballPlayByPlayRenderers.createBasketballRecentPlaysModule(any(), any()))
            .thenReturn(BoxScoreFixture.TestFeedModule(BASKETBALL_PLAY_BY_PLAY_MODULE_ID))
    ...
    @Test
    fun `when a game is scheduled, only this game's state feed modules are created in the correct order`() {
        val list = renderer.renderModules(BoxScoreFixture.boxScoreStateFixture(GameStatus.SCHEDULED, Sport.BASKETBALL))
    ...
    fun `when a game is in progress, only this game's state feed modules are created and in the correct order`() {
        val list = renderer.renderModules(BoxScoreFixture.boxScoreStateFixture(GameStatus.IN_PROGRESS, Sport.BASKETBALL))

        assertThat(list.map { it.moduleId }).isEqualTo(
            listOf(
                SCORING_MODULE_ID,
                BASKETBALL_PLAY_BY_PLAY_MODULE_ID,
                TOP_COMMENTS_MODULE_ID,
    ...
    fun `when a game is completed, only this game's state feed modules are created and in the correct order`() {
        val list = renderer.renderModules(BoxScoreFixture.boxScoreStateFixture(GameStatus.FINAL, Sport.BASKETBALL))
