...
import com.theathletic.gamedetail.boxscore.ui.BASEBALL_CURRENT_INNINGS_MODULE_ID
import com.theathletic.gamedetail.boxscore.ui.BASEBALL_PITCHER_WIN_LOSE_MODULE_ID
import com.theathletic.gamedetail.boxscore.ui.BASEBALL_STARTING_PITCHER_MODULE_ID
...
class BoxScoreBaseballRendererTest {

    private lateinit var renderer: BoxScoreBaseballRenderer
    ...
    private val mockPitchersWinLossRenderer: BoxScoreBaseballPitchersWinLossRenderer = mock()
    private val mockCurrentInningRenderer: BoxScoreBaseballCurrentInningRenderer = mock()
    private val mockPlayerGradeRenderers: BoxScorePlayerGradeRenderers = mock()
    private val mockBaseballPitcherRenderer: BoxScoreBaseballPitcherRenderer = mock()
    ...
    fun setup() {
        ...
        renderer = BoxScoreBaseballRenderer(
        ...
        whenever(mockCurrentInningRenderer.createCurrentInningModule(any()))
            .thenReturn(BoxScoreFixture.TestFeedModule(BASEBALL_CURRENT_INNINGS_MODULE_ID))
        whenever(mockPitchersWinLossRenderer.createPitcherWinLossModule(any()))
            .thenReturn(BoxScoreFixture.TestFeedModule(BASEBALL_PITCHER_WIN_LOSE_MODULE_ID))
        ...
        whenever(mockFeatureSwitches.isFeatureEnabled(FeatureSwitch.PLAYER_GRADES_BASEBALL))
            .thenReturn(true)
        whenever(mockBaseballPitcherRenderer.createPitcherModule(any()))
            .thenReturn(BoxScoreFixture.TestFeedModule(BASEBALL_STARTING_PITCHER_MODULE_ID))
    ...
    fun `when a game is scheduled, only this game's state feed modules are created and in the correct order`() {
        val list = renderer.renderModules(BoxScoreFixture.boxScoreStateFixture(GameStatus.SCHEDULED, Sport.BASEBALL))

        Truth.assertThat(list.map { it.moduleId }).isEqualTo(
            listOf(
                TICKETS_MODULE_ID,
                GAME_ODDS_MODULE_ID,
                BASEBALL_STARTING_PITCHER_MODULE_ID,
    ...
    fun `when a game is in progress, only this game's state feed modules are created and in the correct order`() {
        val list = renderer.renderModules(BoxScoreFixture.boxScoreStateFixture(GameStatus.IN_PROGRESS, Sport.BASEBALL))

        Truth.assertThat(list.map { it.moduleId }).isEqualTo(
            listOf(
                SCORING_MODULE_ID,
                BASEBALL_CURRENT_INNINGS_MODULE_ID,
    ...
    fun `when a game is completed, only this game's state feed modules are created and in the correct order`() {
        val list = renderer.renderModules(BoxScoreFixture.boxScoreStateFixture(GameStatus.FINAL, Sport.BASEBALL))

        Truth.assertThat(list.map { it.moduleId }).isEqualTo(
            listOf(
                BASEBALL_PITCHER_WIN_LOSE_MODULE_ID,
                SCORING_MODULE_ID,
                TOP_COMMENTS_MODULE_ID,
                TOP_PERFORMERS_MODULE_ID,
                TEAM_STATS_MODULE_ID,
