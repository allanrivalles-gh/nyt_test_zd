...
class BoxScoreHockeyRenderer @AutoKoin constructor(
    ...
    fun renderModules(data: BoxScoreState): List<FeedModuleV2> {
        ...
        return listOfNotNull(
            ...
            addHockeyScoringPlays(game, pageOrder),
    ...
    private fun addHockeyScoringPlays(
        game: GameDetailLocalModel,
        pageOrder: AtomicInteger
    ): FeedModuleV2? {
        if (game.isGameInProgressOrCompleted.not()) return null
        pageOrder.getAndIncrement()
        return scoringRenderers.createHockeyScoringSummaryModule(game)
    }
    ...
    private fun addRecentPlaysModule(
        ...
        val extras = game.sportExtras as? GameDetailLocalModel.HockeyExtras ?: return null
        return extras.recentPlays.transformIfNotEmptyElseNull { recentPlays ->
            pageOrder.incrementAndGet()
            hockeyPlayByPlayRenderers.createHockeyRecentPlaysModule(
                game = game,
