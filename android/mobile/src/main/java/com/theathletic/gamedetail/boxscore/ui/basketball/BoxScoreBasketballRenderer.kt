...
class BoxScoreBasketballRenderer @AutoKoin constructor(
    ...
    fun renderModules(data: BoxScoreState): List<FeedModuleV2> {
        ...
        return listOfNotNull(
            ...
            addBasketballRecentPlaysModule(game, pageOrder),
            addTopCommentsIfNecessary(
                game,
                pageOrder,
    ...
    private fun addTopCommentsIfNecessary(
        ...
        if (featureSwitches.isFeatureEnabled(FeatureSwitch.BOX_SCORE_TOP_COMMENTS).not()) return null
    ...
    private fun addBasketballRecentPlaysModule(
        game: GameDetailLocalModel,
        pageOrder: AtomicInteger
    ): FeedModuleV2? {
        if (!game.isGameInProgress) return null
        val extras = game.sportExtras as? GameDetailLocalModel.BasketballExtras ?: return null
        return extras.recentPlays.transformIfNotEmptyElseNull { recentPlays ->
            pageOrder.incrementAndGet()
            basketballPlayByPlayRenderers.createBasketballRecentPlaysModule(
                game,
                recentPlays
            )
        }
