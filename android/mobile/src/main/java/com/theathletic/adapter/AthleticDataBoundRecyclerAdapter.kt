 ...
 * works with [AthleticDataBoundRecyclerViewHolder].
 *
 *
...
abstract class AthleticDataBoundRecyclerAdapter<T : ViewDataBinding>(
    private val view: BaseView
) :
    RecyclerView.Adapter<AthleticDataBoundRecyclerViewHolder<T>>() {
     ...
     */
    private val onRebindCallback: OnRebindCallback<*> = object : OnRebindCallback<ViewDataBinding>() {
    ...
    protected abstract fun bindItem(
        holder: AthleticDataBoundRecyclerViewHolder<T>,
        position: Int,
        payloads: List<Any>?
    )
    ...
    @CallSuper
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): AthleticDataBoundRecyclerViewHolder<T> {
        if (layoutInflater == null) {
            layoutInflater = LayoutInflater.from(parent.context)
        }

        return AthleticDataBoundRecyclerViewHolder.create<T>(view.viewLifecycleOwnerProducer(), layoutInflater!!, parent, viewType)
            .apply { binding.addOnRebindCallback(onRebindCallback) }
    }
    ...
    /**
     * TODO think about whether this is good solution, because if calling this method outside of adapter, it will give "useless" integer numbers
     */
